...
simxChar _wholeThingInitialized=0;
simxInt _clientsCount=0;
simxInt _clientIDForThread;

simxChar _softLock_=0;
simxVoid _softLock()
...
{ 
	simxInt i;
	_softLock(); /* simple and not fail-safe. Init/deinit routines would probably be better, but what about back-compatibility? */
...
simxChar* _setLastFetchedCmd(simxInt clientID,simxChar* cmdPtr,simxInt* error)
{
	...
	if (cmdPtr!=0)
	{
		...
		{ /* we need more memory here for the fetched command buffer */
			...
			newCommand=(simxChar*)extApi_allocateBuffer(_commandReceived_bufferSize[clientID]+incr);
			extApi_releaseBuffer(_commandReceived[clientID]);
...
simxChar* _appendCommand_null_buff(simxInt cmd,simxChar options,simxChar* buffer,simxInt bufferSize,simxUShort delayOrSplit,simxChar* destBuffer,simxInt* destBuffer_bufferSize,simxInt* destBuffer_dataSize)
{
	simxInt i;
	simxChar* retVal;
	simxChar* data=extApi_allocateBuffer(SIMX_SUBHEADER_SIZE+0+bufferSize);
...
EXTAPI_DLLEXPORT simxInt simxGetInMessageInfo(simxInt clientID,simxInt infoType,simxInt* info)
{
	...
	if (_messageReceived_dataSize[clientID]>=SIMX_HEADER_SIZE)
	{
		...
		{
			info[0]=(simxInt)((simxChar*)(_messageReceived[clientID]+infoType))[0];
			retVal=1;
		}
	}
	extApi_unlockResources(clientID);
...
EXTAPI_DLLEXPORT simxInt simxSetSphericalJointMatrix(simxInt clientID,simxInt jointHandle,simxFloat* matrix,simxInt operationMode)
{
	...
	_exec_int_buffer(clientID,simx_cmd_set_spherical_joint_matrix,operationMode,0,jointHandle,(simxChar*)matrix,4*12,&returnValue);
	return(returnValue);
}
...
EXTAPI_DLLEXPORT simxInt simxGetObjectHandle(simxInt clientID,const simxChar* objectName,simxInt* handle,simxInt operationMode)
{
	...
	if (_communicationThreadRunning[clientID]==0)
		return(simx_error_initialize_error_flag);
	if (operationMode==simx_opmode_remove)
		return(_removeCommandReply_string(clientID,simx_cmd_get_object_handle,objectName));
	dataPointer=_exec_string(clientID,simx_cmd_get_object_handle,operationMode,0,objectName,&returnValue);
	if ((dataPointer!=0)&&(returnValue==0))
...
EXTAPI_DLLEXPORT simxInt simxGetUIHandle(simxInt clientID,const simxChar* uiName,simxInt* handle,simxInt operationMode)
{
	...
	if (_communicationThreadRunning[clientID]==0)
		return(simx_error_initialize_error_flag);
	if (operationMode==simx_opmode_remove)
		return(_removeCommandReply_string(clientID,simx_cmd_get_ui_handle,uiName));
	dataPointer=_exec_string(clientID,simx_cmd_get_ui_handle,operationMode,0,uiName,&returnValue);
	if ((dataPointer!=0)&&(returnValue==0))
		handle[0]=_readPureDataInt(dataPointer,0,0);
	return(returnValue);
}
...
EXTAPI_DLLEXPORT simxInt simxTransferFile(simxInt clientID,const simxChar* filePathAndName,const simxChar* fileName_serverSide,simxInt timeOut,simxInt operationMode)
{
	...
	if (_communicationThreadRunning[clientID]==0)
		return(simx_error_initialize_error_flag);
	if (operationMode==simx_opmode_remove)
		return(_removeCommandReply_string(clientID,simx_cmd_transfer_file,filePathAndName));
	...
	_exec_string_buffer(clientID,simx_cmd_transfer_file,operationMode,0,fileName_serverSide,buffer,bufferLength,&returnValue);
...
EXTAPI_DLLEXPORT simxInt simxEraseFile(simxInt clientID,const simxChar* fileName_serverSide,simxInt operationMode)
{
	...
	if (_communicationThreadRunning[clientID]==0)
		return(simx_error_initialize_error_flag);
	if (operationMode==simx_opmode_remove)
		return(_removeCommandReply_string(clientID,simx_cmd_erase_file,fileName_serverSide));
	_exec_string(clientID,simx_cmd_erase_file,operationMode,0,fileName_serverSide,&returnValue);
	return(returnValue);
}
...
EXTAPI_DLLEXPORT simxInt simxLoadModel(simxInt clientID,const simxChar* modelPathAndName,simxChar options,simxInt* baseHandle,simxInt operationMode)
{
	...
	if (_communicationThreadRunning[clientID]==0)
		return(simx_error_initialize_error_flag);
	if (operationMode==simx_opmode_remove)
		return(_removeCommandReply_string(clientID,simx_cmd_load_model,modelPathAndName));
	if (options&1)
	{ /* give some pseudo-random name to the temp file: */
		...
		if (returnValue==0)
		{
			dataPointer=_exec_string(clientID,simx_cmd_load_model,operationMode,0,tmpFileName,&returnValue);
		...
		dataPointer=_exec_string(clientID,simx_cmd_load_model,operationMode,0,modelPathAndName,&returnValue);
...
EXTAPI_DLLEXPORT simxInt simxLoadUI(simxInt clientID,const simxChar* uiPathAndName,simxChar options,simxInt* count,simxInt** uiHandles,simxInt operationMode)
{
	...
	if (_communicationThreadRunning[clientID]==0)
		return(simx_error_initialize_error_flag);
	if (operationMode==simx_opmode_remove)
		return(_removeCommandReply_string(clientID,simx_cmd_load_ui,uiPathAndName));
	if (options&1)
	{ /* give some pseudo-random name to the temp file: */
		tmpFileName[20]='0'+(char)(extApi_rand()*9.1f);
		tmpFileName[21]='0'+(char)(extApi_rand()*9.1f);
		...
		if (returnValue==0)
		{
			dataPointer=_exec_string(clientID,simx_cmd_load_ui,operationMode,0,tmpFileName,&returnValue);
			simxEraseFile(clientID,tmpFileName,simx_opmode_oneshot);
		}
		simxTransferFile(clientID,uiPathAndName,tmpFileName,60000,simx_opmode_remove);
	}
	else
		dataPointer=_exec_string(clientID,simx_cmd_load_ui,operationMode,0,uiPathAndName,&returnValue);
	if ((dataPointer!=0)&&(returnValue==0))
	{
...
EXTAPI_DLLEXPORT simxInt simxLoadScene(simxInt clientID,const simxChar* scenePathAndName,simxChar options,simxInt operationMode)
{
	...
	if (_communicationThreadRunning[clientID]==0)
		return(simx_error_initialize_error_flag);
	if (operationMode==simx_opmode_remove)
		return(_removeCommandReply_string(clientID,simx_cmd_load_scene,scenePathAndName));
	if (options&1)
	{ /* give some pseudo-random name to the temp file: */
		tmpFileName[20]='0'+(char)(extApi_rand()*9.1f);
		tmpFileName[21]='0'+(char)(extApi_rand()*9.1f);
		...
		if (returnValue==0)
		{
			_exec_string(clientID,simx_cmd_load_scene,operationMode,0,tmpFileName,&returnValue);
			simxEraseFile(clientID,tmpFileName,simx_opmode_oneshot);
		}
		simxTransferFile(clientID,scenePathAndName,tmpFileName,60000,simx_opmode_remove);
	}
	else
		_exec_string(clientID,simx_cmd_load_scene,operationMode,0,scenePathAndName,&returnValue);
	return(returnValue);
}
...
EXTAPI_DLLEXPORT simxInt simxAddStatusbarMessage(simxInt clientID,const simxChar* message,simxInt operationMode)
{
	...
	if (_communicationThreadRunning[clientID]==0)
		return(simx_error_initialize_error_flag);
	if (operationMode==simx_opmode_remove)
		return(_removeCommandReply_string(clientID,simx_cmd_add_statusbar_message,message));
	_exec_string(clientID,simx_cmd_add_statusbar_message,operationMode,0,message,&returnValue);
	return(returnValue);
}
...
EXTAPI_DLLEXPORT simxInt simxCreateDummy(simxInt clientID,simxFloat size,const simxChar* colors,simxInt* objectHandle,simxInt operationMode)
{
	...
	dataPointer=_exec_null_buffer(clientID,simx_cmd_create_dummy,operationMode,1,buffer,13*4,&returnValue);
	if ((dataPointer!=0)&&(returnValue==0))
...
EXTAPI_DLLEXPORT simxInt simxAuxiliaryConsoleOpen(simxInt clientID,const simxChar* title,simxInt maxLines,simxInt mode,simxInt* position,simxInt* size,simxFloat* textColor,simxFloat* backgroundColor,simxInt* consoleHandle,simxInt operationMode)
{
	...
	if (_communicationThreadRunning[clientID]==0)
		return(simx_error_initialize_error_flag);
	if (operationMode==simx_opmode_remove)
		return(_removeCommandReply_string(clientID,simx_cmd_aux_console_open,title));
	...
	dataPointer=_exec_string_buffer(clientID,simx_cmd_aux_console_open,operationMode,0,title,buffer,12*4,&returnValue);
	if ((dataPointer!=0)&&(returnValue==0))
...
EXTAPI_DLLEXPORT simxInt simxAuxiliaryConsolePrint(simxInt clientID,simxInt consoleHandle,const simxChar* txt,simxInt operationMode)
{
	...
	if (txt==0)
		...
		_exec_int_buffer(clientID,simx_cmd_aux_console_print,operationMode,1,consoleHandle,(simxChar*)txt,extApi_getStringLength(txt)+1,&returnValue);
	return(returnValue);
}
...
EXTAPI_DLLEXPORT simxInt simxSetArrayParameter(simxInt clientID,simxInt paramIdentifier,const simxFloat* paramValues,simxInt operationMode)
{
	...
	_exec_int_buffer(clientID,simx_cmd_set_array_parameter,operationMode,0,paramIdentifier,(simxChar*)paramValues,3*4,&returnValue);
	return(returnValue);
}
...
EXTAPI_DLLEXPORT simxInt simxGetCollisionHandle(simxInt clientID,const simxChar* collisionObjectName,simxInt* handle,simxInt operationMode)
{
	...
	if (_communicationThreadRunning[clientID]==0)
		return(simx_error_initialize_error_flag);
	if (operationMode==simx_opmode_remove)
		return(_removeCommandReply_string(clientID,simx_cmd_get_collision_handle,collisionObjectName));
	dataPointer=_exec_string(clientID,simx_cmd_get_collision_handle,operationMode,0,collisionObjectName,&returnValue);
	if ((dataPointer!=0)&&(returnValue==0))
		handle[0]=_readPureDataInt(dataPointer,0,0);
	return(returnValue);
}
...
EXTAPI_DLLEXPORT simxInt simxGetDistanceHandle(simxInt clientID,const simxChar* distanceObjectName,simxInt* handle,simxInt operationMode)
{
	...
	if (_communicationThreadRunning[clientID]==0)
		return(simx_error_initialize_error_flag);
	if (operationMode==simx_opmode_remove)
		return(_removeCommandReply_string(clientID,simx_cmd_get_distance_handle,distanceObjectName));
	dataPointer=_exec_string(clientID,simx_cmd_get_distance_handle,operationMode,0,distanceObjectName,&returnValue);
	if ((dataPointer!=0)&&(returnValue==0))
		handle[0]=_readPureDataInt(dataPointer,0,0);
	return(returnValue);
}
...
EXTAPI_DLLEXPORT simxInt simxDisplayDialog(simxInt clientID,const simxChar* titleText,const simxChar* mainText,simxInt dialogType,const simxChar* initialText,simxFloat* titleColors,simxFloat* dialogColors,simxInt* dialogHandle,simxInt* uiHandle,simxInt operationMode)
{
	...
	if (_communicationThreadRunning[clientID]==0)
		return(simx_error_initialize_error_flag);
	if (operationMode==simx_opmode_remove)
		return(_removeCommandReply_string(clientID,simx_cmd_display_dialog,titleText));
	...
	dataPointer=_exec_string_buffer(clientID,simx_cmd_display_dialog,operationMode,0,titleText,buffer,str1L+1+4+str2L+1+4*6+4*6,&returnValue);
	extApi_releaseBuffer(buffer);
	if ((dataPointer!=0)&&(returnValue==0))
	{
...
EXTAPI_DLLEXPORT simxInt simxCopyPasteObjects(simxInt clientID,const simxInt* objectHandles,simxInt objectCount,simxInt** newObjectHandles,simxInt* newObjectCount,simxInt operationMode)
{
	...
	dataPointer=_exec_int_buffer(clientID,simx_cmd_copy_paste_objects,operationMode,0,0,(simxChar*)objectHandles,objectCount*4,&returnValue);
	if ((dataPointer!=0)&&(returnValue==0))
	{
...
EXTAPI_DLLEXPORT simxInt simxSetObjectSelection(simxInt clientID,const simxInt* objectHandles,simxInt objectCount,simxInt operationMode)
{
	...
	_exec_int_buffer(clientID,simx_cmd_set_object_selection,operationMode,0,0,(simxChar*)objectHandles,objectCount*4,&returnValue);
	return(returnValue);
}
...
EXTAPI_DLLEXPORT simxInt simxClearFloatSignal(simxInt clientID,const simxChar* signalName,simxInt operationMode)
{
	...
	if (_communicationThreadRunning[clientID]==0)
		return(simx_error_initialize_error_flag);
	if (operationMode==simx_opmode_remove)
		return(_removeCommandReply_string(clientID,simx_cmd_clear_float_signal,signalName));
	_exec_string(clientID,simx_cmd_clear_float_signal,operationMode,0,signalName,&returnValue);
	return(returnValue);
}
...
EXTAPI_DLLEXPORT simxInt simxClearIntegerSignal(simxInt clientID,const simxChar* signalName,simxInt operationMode)
{
	...
	if (_communicationThreadRunning[clientID]==0)
		return(simx_error_initialize_error_flag);
	if (operationMode==simx_opmode_remove)
		return(_removeCommandReply_string(clientID,simx_cmd_clear_integer_signal,signalName));
	_exec_string(clientID,simx_cmd_clear_integer_signal,operationMode,0,signalName,&returnValue);
	return(returnValue);
}
...
EXTAPI_DLLEXPORT simxInt simxClearStringSignal(simxInt clientID,const simxChar* signalName,simxInt operationMode)
{
	...
	if (_communicationThreadRunning[clientID]==0)
		return(simx_error_initialize_error_flag);
	if (operationMode==simx_opmode_remove)
		return(_removeCommandReply_string(clientID,simx_cmd_clear_string_signal,signalName));
	_exec_string(clientID,simx_cmd_clear_string_signal,operationMode,0,signalName,&returnValue);
	return(returnValue);
}
...
EXTAPI_DLLEXPORT simxInt simxGetFloatSignal(simxInt clientID,const simxChar* signalName,simxFloat* signalValue,simxInt operationMode)
{
	...
	if (_communicationThreadRunning[clientID]==0)
		return(simx_error_initialize_error_flag);
	if (operationMode==simx_opmode_remove)
		return(_removeCommandReply_string(clientID,simx_cmd_get_float_signal,signalName));
	dataPointer=_exec_string(clientID,simx_cmd_get_float_signal,operationMode,0,signalName,&returnValue);
	if ((dataPointer!=0)&&(returnValue==0))
...
EXTAPI_DLLEXPORT simxInt simxGetIntegerSignal(simxInt clientID,const simxChar* signalName,simxInt* signalValue,simxInt operationMode)
{
	...
	if (_communicationThreadRunning[clientID]==0)
		return(simx_error_initialize_error_flag);
	if (operationMode==simx_opmode_remove)
		return(_removeCommandReply_string(clientID,simx_cmd_get_integer_signal,signalName));
	dataPointer=_exec_string(clientID,simx_cmd_get_integer_signal,operationMode,0,signalName,&returnValue);
	if ((dataPointer!=0)&&(returnValue==0))
...
EXTAPI_DLLEXPORT simxInt simxGetStringSignal(simxInt clientID,const simxChar* signalName,simxChar** signalValue,simxInt* signalLength,simxInt operationMode)
{
	...
	if (_communicationThreadRunning[clientID]==0)
		return(simx_error_initialize_error_flag);
	if (operationMode==simx_opmode_remove)
		return(_removeCommandReply_string(clientID,simx_cmd_get_string_signal,signalName));
	dataPointer=_exec_string(clientID,simx_cmd_get_string_signal,operationMode,0,signalName,&returnValue);
	if ((dataPointer!=0)&&(returnValue==0))
	{
...
EXTAPI_DLLEXPORT simxInt simxGetAndClearStringSignal(simxInt clientID,const simxChar* signalName,simxChar** signalValue,simxInt* signalLength,simxInt operationMode)
{
	...
	if (_communicationThreadRunning[clientID]==0)
		return(simx_error_initialize_error_flag);
	if (operationMode==simx_opmode_remove)
		return(_removeCommandReply_string(clientID,simx_cmd_get_and_clear_string_signal,signalName));
	dataPointer=_exec_string(clientID,simx_cmd_get_and_clear_string_signal,operationMode,0,signalName,&returnValue);
	if ((dataPointer!=0)&&(returnValue==0))
	{
		signalValue[0]=dataPointer+SIMX_SUBHEADER_SIZE+_getCmdDataSize(dataPointer);
		signalLength[0]=extApi_endianConversionInt(((simxInt*)(dataPointer+simx_cmdheaderoffset_full_mem_size))[0])-SIMX_SUBHEADER_SIZE-_getCmdDataSize(dataPointer);
		...
		if (operationMode==simx_opmode_buffer)/* &&(signalLength[0]>0)) */
			_removeCommandReply_string(clientID,simx_cmd_get_and_clear_string_signal,signalName); // We received a signal value! The continuous command was automatically deactivated on the server side. We remove the reply  in the input buffer on the client here!
		//******************************************************
...
EXTAPI_DLLEXPORT simxInt simxSetFloatSignal(simxInt clientID,const simxChar* signalName,simxFloat signalValue,simxInt operationMode)
{
	...
	if (_communicationThreadRunning[clientID]==0)
		return(simx_error_initialize_error_flag);
	if (operationMode==simx_opmode_remove)
		return(_removeCommandReply_string(clientID,simx_cmd_set_float_signal,signalName));
	signalValue=extApi_endianConversionFloat(signalValue);
	_exec_string_buffer(clientID,simx_cmd_set_float_signal,operationMode,0,signalName,(simxChar*)&signalValue,4,&returnValue);
	return(returnValue);
}
...
EXTAPI_DLLEXPORT simxInt simxSetIntegerSignal(simxInt clientID,const simxChar* signalName,simxInt signalValue,simxInt operationMode)
{
	...
	if (_communicationThreadRunning[clientID]==0)
		return(simx_error_initialize_error_flag);
	if (operationMode==simx_opmode_remove)
		return(_removeCommandReply_string(clientID,simx_cmd_set_integer_signal,signalName));
	signalValue=extApi_endianConversionInt(signalValue);
	_exec_string_buffer(clientID,simx_cmd_set_integer_signal,operationMode,0,signalName,(simxChar*)&signalValue,4,&returnValue);
	return(returnValue);
}
...
EXTAPI_DLLEXPORT simxInt simxSetStringSignal(simxInt clientID,const simxChar* signalName,const simxChar* signalValue,simxInt signalLength,simxInt operationMode)
{
	...
	if (_communicationThreadRunning[clientID]==0)
		return(simx_error_initialize_error_flag);
	if (operationMode==simx_opmode_remove)
		return(_removeCommandReply_string(clientID,simx_cmd_set_string_signal,signalName));
	_exec_string_buffer(clientID,simx_cmd_set_string_signal,operationMode,0,signalName,(simxChar*)signalValue,signalLength,&returnValue);
	return(returnValue);
}
...
{ /* since 31.1.2013: append mode */
	...
	if (_communicationThreadRunning[clientID]==0)
		return(simx_error_initialize_error_flag);
	if (operationMode==simx_opmode_remove)
		return(_removeCommandReply_string(clientID,simx_cmd_append_string_signal,signalName)); 
	_exec_string_buffer(clientID,simx_cmd_append_string_signal,operationMode,1,signalName,(simxChar*)signalValue,signalLength,&returnValue);
	return(returnValue);
}
...
EXTAPI_DLLEXPORT simxInt simxSetObjectFloatParameter(simxInt clientID,simxInt objectHandle,simxInt parameterID,simxFloat parameterValue,simxInt operationMode)
{
	...
	_exec_intint_buffer(clientID,simx_cmd_set_object_float_parameter,operationMode,0,objectHandle,parameterID,(simxChar*)&parameterValue,4,&returnValue);
	return(returnValue);
}
...
EXTAPI_DLLEXPORT simxInt simxSetObjectIntParameter(simxInt clientID,simxInt objectHandle,simxInt parameterID,simxInt parameterValue,simxInt operationMode)
{
	...
	_exec_intint_buffer(clientID,simx_cmd_set_object_int_parameter,operationMode,0,objectHandle,parameterID,(simxChar*)&parameterValue,4,&returnValue);
	return(returnValue);
}
...
EXTAPI_DLLEXPORT simxInt simxRMLPosition(simxInt clientID,simxInt dofs,simxDouble timeStep,simxInt flags,const simxDouble* currentPosVelAccel,const simxDouble* maxVelAccelJerk,const simxChar* selection,const simxDouble* targetPosVel,simxInt* rmlState,simxDouble* newPosVelAccel,simxInt operationMode)
{
	...
	if (_communicationThreadRunning[clientID]==0)
		return(simx_error_initialize_error_flag);
	if (operationMode==simx_opmode_remove)
		return(_removeCommandReply_null(clientID,simx_cmd_rml_position));
	...
	dataPointer=_exec_int_buffer(clientID,simx_cmd_rml_position,operationMode,0,0,dataBuffer,dataBufferSize,&returnValue);
...
EXTAPI_DLLEXPORT simxInt simxRMLVelocity(simxInt clientID,simxInt dofs,simxDouble timeStep,simxInt flags,const simxDouble* currentPosVelAccel,const simxDouble* maxAccelJerk,const simxChar* selection,const simxDouble* targetVel,simxInt* rmlState,simxDouble* newPosVelAccel,simxInt operationMode)
{
	...
	if (_communicationThreadRunning[clientID]==0)
		return(simx_error_initialize_error_flag);
	if (operationMode==simx_opmode_remove)
		return(_removeCommandReply_null(clientID,simx_cmd_rml_velocity));
	...
	dataPointer=_exec_int_buffer(clientID,simx_cmd_rml_velocity,operationMode,0,0,dataBuffer,dataBufferSize,&returnValue);
	extApi_releaseBuffer(dataBuffer);
...
EXTAPI_DLLEXPORT simxInt simxQuery(simxInt clientID,const simxChar* signalName,const simxChar* signalValue,simxInt signalLength,const simxChar* retSignalName,simxChar** retSignalValue,simxInt* retSignalLength,simxInt timeOutInMs)
{
	...
	_removeCommandReply_string(clientID,simx_cmd_get_and_clear_string_signal,retSignalName); /* just in case */
...
EXTAPI_DLLEXPORT simxInt simxGetObjectGroupData(simxInt clientID,simxInt objectType,simxInt dataType,simxInt* handlesCount,simxInt** handles,simxInt* intDataCount,simxInt** intData,simxInt* floatDataCount,simxFloat** floatData,simxInt* stringDataCount,simxChar** stringData,simxInt operationMode)
{
	...
	if ((dataPointer!=0)&&(returnValue==0))
	{
		...
		if (stringData!=NULL)
			stringData[0]=dataPointer+SIMX_SUBHEADER_SIZE+_getCmdDataSize(dataPointer)+additionalOffset;
	}
	return(returnValue);
}
...
JNIEXPORT jint JNICALL Java_coppelia_remoteApi_simxStart(JNIEnv * env, jobject obj, jstring connAddr, jint connPort, jboolean waitUntilConnected, jboolean doNotRecon, jint toInMs, jint commThreadCycleInMs)
{
	...
	simxChar waitUntilConnected_ = waitUntilConnected ? 1 : 0;
...
JNIEXPORT jint JNICALL Java_coppelia_remoteApi_simxSetVisionSensorImage(JNIEnv *env, jobject obj, jint clientID, jint hdl, jobject img, jint bufsize, jint opts, jint opMode)
{
	...
	simxInt retVal = simxSetVisionSensorImage(theClientID,sensorHandle,  (simxChar*)imgbuf, bufferSize, options, operationMode);
	delete[] imgbuf; 
...
JNIEXPORT jint JNICALL Java_coppelia_remoteApi_simxQuery(JNIEnv *env, jobject obj, jint clientID, jstring sn, jstring sv,jstring rsn, jobject rsv, jint timeOutInMs)
{
	...
	if (retVal==0)
	{
		if (retSignalLength>0)
		{
			...
			jmethodID mid = env->GetMethodID(cls, "setValue", "(Ljava/lang/String;)V");
			simxChar* sigValue = new simxChar[retSignalLength];
			substr( retSignalValue, sigValue, 0, retSignalLength);
